...
const Messenger = ({ user }: { user: any }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (activeChat.isBot) {
      ...
      // Simulate typing
      setTimeout(() => setBotIsTyping(true), 500);
      
      setTimeout(async () => {
        const botReply = await generateBotResponse(activeChat.displayName, activeChat.bio, text);
        await sendMessage(chatId, activeChat.id, botReply);
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-zinc-500 truncate">
                  {chat.isBot ? "AI Companion" : "Click to chat"}
                </p>
              </div>
            </div>
          ))}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-zinc-50/30">
              ...
              {botIsTyping && (
                <div className="flex justify-start">
                  <div className="bg-white border border-zinc-100 px-4 py-3 rounded-2xl rounded-tl-none shadow-sm flex gap-1">
